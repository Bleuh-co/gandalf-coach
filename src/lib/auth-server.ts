...
/**
 * Résout l'ID Firestore de l'app Gandalf Coach dans la collection `apps`.
 *
 * Priorité 1 : variable d'env GANDACOACH_APP_ID (direct, zéro ambiguïté).
 * Priorité 2 : matcher par nom dans la collection apps (fallback dev).
 */
async function resolveGandaCoachAppId(
  db: FirebaseFirestore.Firestore
): Promise<{ appId: string; appName: string | null }> {
  const override = process.env.GANDACOACH_APP_ID;
  if (override) {
  ...
  const match = appsSnap.docs.find((d) => {
    ...
    return name.includes("gandalfcoach") || (name.includes("gandalf") && name.includes("coach"));
  });
...
export async function resolveRoleVerbose(email: string): Promise<RoleResolution> {
  ...
  try {
    const { appId, appName } = await resolveGandaCoachAppId(db);
    base.appId = appId;
